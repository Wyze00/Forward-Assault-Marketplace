...
const captureData = async () => {
  ...
  try {
    ...
    // Refresh history after successful capture
    await refresh()
...
const getConditionData = (cond) => {
  if (cond == 0) return { name: 'Factory New', color: '#d1ecf1', text: '#0c5460' } // Blue 
  if (cond < 0.15) return { name: 'Minimal Wear', color: '#e2d9f3', text: '#5e35b1' } // Purple
  if (cond < 0.35) return { name: 'Field Tested', color: '#fff3cd', text: '#856404' } // Yellow
  if (cond < 0.45) return { name: 'Well Worn', color: '#F28500', text: '#CC5500' } // Red
  return { name: 'Battle Scarred', color: '#ffb3b3', text: '#900' } // Red
}
